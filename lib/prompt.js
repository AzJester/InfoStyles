...
export function sanitizePrompt(input = {}) {
  ...
  // be text, uploaded images (image-generation prompts), or both. `images`
  // holds app-served /uploads/ paths (or absolute URLs); the key exists only
  // when there are images, so text-only records round-trip unchanged.
  ...
  out.results = results
    ...
    .filter((r) => r.output || r.images)
    .slice(0, 50);
  return out;
}

// Image URLs attached to a saved output: uploaded files served from /uploads,
// or an absolute http(s) URL. Anything else (javascript:, data: blobs that
// would bloat the store, path traversal) is dropped.
const IMAGE_URL_RE = /^(\/uploads\/|https?:\/\/)/i;
