...
// Read-only check (no increment) so the UI can show the out-of-tokens
// disclaimer before the visitor even tries.
export async function getStudioDaily() {
  if (!kvAvailable()) return 0;
  const n = await redis().get(`studio:daily:${new Date().toISOString().slice(0, 10)}`);
  return Number(n) || 0;
}
